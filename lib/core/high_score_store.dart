...
class HighScoreStore {
  ...
  static Future<void> saveHighScore(String mode, int score, int level, String name) async {
    final prefs = await SharedPreferences.getInstance();
  ...
  static Future<List<Map<String, dynamic>>> getAllHighScores() async {
    ...
    for (final mode in ['classic', 'arena']) {
      ...
      if (legacyScore != null && legacyScore > 0) {
        ...
        bool found = list.any((e) => e['mode'] == mode && e['score'] == legacyScore && e['name'] == legacyName);
        if (!found) {
          list.add({
            'name': legacyName,
            'mode': mode,
